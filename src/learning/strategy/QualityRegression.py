...
def _register_regression(x_dim, y_dim):
    return [
        (ModelType.train_only, ScikitBayesianRidge.ScikitBayesianRidge(combine_name(QualityRegression.__name__, ScikitBayesianRidge.ScikitBayesianRidge.__name__))),
        (ModelType.train_only, ScikitSvm.ScikitSvm('rbf', combine_name(QualityRegression.__name__, ScikitSvm.ScikitSvm.__name__))),
        (ModelType.train_validation, TheanetsANNs.TheanetsANNs(x_dim, y_dim, math.floor(x_dim / 2), combine_name(QualityRegression.__name__, TheanetsANNs.TheanetsANNs.__name__))),
        (ModelType.train_only, PyBrainANNs.PyBrainANNs(x_dim, y_dim, math.floor(x_dim / 2), combine_name(QualityRegression.__name__, PyBrainANNs.PyBrainANNs.__name__)))
    ]
